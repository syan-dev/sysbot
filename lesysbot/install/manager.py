...
(``tools.lock.json``) so ``lesysbot tools list/info/remove`` know where an
installed package came from.

Installing is all this module does. Reading back what is installed belongs to
the registry — ``lesysbot tools list/info`` (``mcp/cli.py``) join live registry
rows with this lock file — and removal is ``registry.remove_tool()`` plus
``lockfile.drop_entries()``. Keeping one path for each avoids two sources of
truth about the tools dir.
"""
...
from lesysbot.install.archive import extract_tree, zip_commit_sha
from lesysbot.install.errors import ToolInstallError
from lesysbot.install.fetch import Fetcher, UrllibFetcher, download_zipball
from lesysbot.install.lockfile import LOCK_KEY, JsonState
from lesysbot.install.meta import NAME_RE, ToolPackage, discover_packages
    ...
    def _select(
        ...
        if not packages:
            raise ToolInstallError(
                ...
                "in the repo root, or per-tool subdirectories there or under `tools/`."
            )
        if not only:
            return packages
    ...
    def _check_collisions(
        ...
        if conflicts and not force:
            raise ToolInstallError(
                ...
                "`lesysbot tools install`) — pass --force to overwrite."
            )
