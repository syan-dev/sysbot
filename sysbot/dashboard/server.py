...
is guarded so the rest of SysBot works without it.
"""
...
class Dashboard:
    ...
    async def start(self) -> None:
        ...
        await site.start()
        logger.info("Dashboard at http://%s:%d", self._cfg.host, self._cfg.port)
        print(f"📊 Dashboard: http://{self._cfg.host}:{self._cfg.port}")
...
<body>
<header><h1>📊 SysBot Dashboard</h1><span class="muted" id="sub"></span></header>
<div class="wrap">
