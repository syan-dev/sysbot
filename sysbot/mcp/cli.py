"""`sysbot tools …` subcommands — install and manage tools from the terminal.
...
def register_subcommands(subparsers: argparse._SubParsersAction) -> None:
    # -c is repeated on each leaf with SUPPRESS so both `sysbot -c x.yaml tools
    # list` and `sysbot tools list -c x.yaml` work.
...
def _install(settings: Settings, console: Console, args: argparse.Namespace) -> int:
    ...
    from sysbot.install.spec import parse_source
    ...
    if "/" not in args.spec and "://" not in args.spec and not args.spec.startswith("git@"):
        ...
        console.print(
            "    sysbot tools install owner/repo[/subdir][@ref]\n"
...
def _missing(console: Console, name: str) -> int:
    console.print(f"[red]Error:[/red] no tool named {name!r} (see `sysbot tools list`)")
    return 1
...
def _list(registry: ToolRegistry, settings: Settings, console: Console, as_json: bool) -> int:
    ...
    if not rows:
        console.print(
            ...
            "`sysbot tools install owner/repo`."
        )
        return 0
...
def _set_enabled(registry: ToolRegistry, console: Console, name: str, enabled: bool) -> int:
    ...
    console.print(
        "[dim]A running SysBot applies this on its next restart "
...
def _remove(
    ...
    console.print(
        "[dim]A running SysBot with hot_reload drops it automatically; otherwise restart.[/dim]"
    )
    return 0
