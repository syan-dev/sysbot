...
from sysbot.core.paths import force_rmtree
from sysbot.mcp.cli_tool import CLITool
from sysbot.mcp.platform import availability
...
class ToolRegistry:
    ...
    def _load_package(self, package: Path) -> None:
        ...
        try:
            for py_file in py_files:
                self._load_file(py_file, prefix=f"_sysbot_tools.{package.name}")
        finally:
            _drop_user_helpers()
            if added and pkg_str in sys.path:
                sys.path.remove(pkg_str)

    def _load_file(self, path: Path, prefix: str = "_sysbot_tools") -> None:
    ...
    def remove_tool(self, name: str) -> dict[str, Any]:
        ...
        unit = Path(info["path"])
        # Belt-and-braces (mirrors ToolInstaller.remove): never delete anything
        # that isn't a direct child of the tools dir.
